...
function useSentimentIndicators() {
    ...
    for (const hl of macroHeadlines) {
        const lower = hl.text.toLowerCase();
        if (['tighten', 'elevated', 'restrictive', 'pressure'].some((k) => lower.includes(k)))
            finScore--;
        if (['ease', 'loosen', 'abundant', 'favorable', 'raises'].some((k) => lower.includes(k)))
            finScore++;
    }
...
export default function MarketScreen() {
    ...
    const mc = moodConfig[sectorMood];
    const fc = finConfig[financingClimate];
    ...
    return (_jsxs("div", { className: "space-y-6 max-w-[1200px]", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-display font-semibold text-text-primary", children: "Market & Signals" }), _jsx("p", { className: "text-[12px] text-text-muted mt-1", children: "External environment and market intelligence" })] }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4", children: [_jsx(Panel, { title: "Sector Mood", children: _jsxs("div", { className: "text-center py-4", children: [_jsxs("div", { className: "flex items-center justify-center gap-2", children: [_jsx(mc.icon, { size: 20, className: mc.color }), _jsx("span", { className: `text-2xl font-mono font-bold ${mc.color}`, children: sectorMood })] }), _jsx("div", { className: "text-[11px] text-text-muted mt-1", children: mc.detail })] }) }), _jsx(Panel, { title: "Financing Climate", children: _jsxs("div", { className: "text-center py-4", children: [_jsxs("div", { className: "flex items-center justify-center gap-2", children: [_jsx(fc.icon, { size: 20, className: fc.color }), _jsx("span", { className: `text-2xl font-mono font-bold ${fc.color}`, children: financingClimate })] }), _jsx("div", { className: "text-[11px] text-text-muted mt-1", children: fc.detail })] }) }), _jsx(Panel, { title: "Buyer Sentiment", children: _jsxs("div", { className: "text-center py-4", children: [_jsx("span", { className: `text-2xl font-mono font-bold ${bc.color}`, children: buyerSentiment }), _jsx("div", { className: "text-[11px] text-text-muted mt-1", children: bc.detail })] }) })] }), _jsx(Panel, { title: "News Feed", subtitle: `${headlines.length} items through week ${week}`, children: sortedHeadlines.length === 0 ? (_jsx("p", { className: "text-[12px] text-text-muted py-8 text-center", children: "No market activity this week." })) : (_jsx("div", { className: "space-y-3", children: sortedHeadlines.map((hl) => (_jsxs("div", { className: "flex items-start gap-3 p-3 rounded-[var(--radius-md)] bg-surface-default border border-border-subtle", children: [_jsx("span", { className: "w-2 h-2 rounded-full bg-accent-primary/60 mt-1.5 shrink-0" }), _jsxs("div", { className: "flex-1", children: [_jsx("p", { className: "text-[13px] text-text-secondary leading-relaxed", children: hl.text }), _jsxs("div", { className: "flex items-center gap-2 mt-1.5", children: [_jsx(StatusChip, { label: hl.category.replace('_', ' ') }), _jsxs("span", { className: "text-[10px] font-mono text-text-muted", children: ["Week ", hl.week] })] })] })] }, hl.id))) })) })] }));
}
//# sourceMappingURL=MarketScreen.js.map