...
import { ArrowRight, Mail, AlertTriangle, ChevronRight, Wallet, Users, Presentation, FileText, Handshake, Trophy, ScrollText } from 'lucide-react';
...
export default function DashboardScreen() {
  ...
  // Phase 0: board approval + engine gate
  // Phase 1: pitch presented + fee agreed + engine gate
  // Phase 2+: engine gate only
    ...
    <div className="space-y-6 max-w-[1200px]">
      ...
      <div className="flex items-baseline justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Staffing button */}
          <button
            onClick={() => setModal('staffing')}
            className="flex items-center gap-1.5 px-3 py-2 border border-border-subtle rounded-[var(--radius-md)] text-[12px] text-text-secondary hover:bg-surface-hover hover:text-text-primary transition-colors"
          >
      ...
      {/* Competitor threat banner (above KPIs) */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <KpiTile
          label="Budget"
          value={`€${resources.budget}k`}
          color={isBudgetLow ? 'danger' : resources.budget < 30 ? 'warning' : 'default'}
      ...
      <div className="rounded-[var(--radius-lg)] border border-border-subtle bg-bg-secondary p-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {phase === 0 ? (
            <>
              {/* Phase 0 → 1 requires board approval + engine gate tasks */}
              <button
                onClick={() => setModal('board')}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-[var(--radius-md)] border text-[12px] font-medium transition-colors ${
                  boardApproved
                    ? 'border-green-500/30 bg-green-500/5 text-green-400'
                    : 'border-accent-primary/40 bg-accent-soft text-text-accent hover:bg-accent-soft/80'
                }`}
              >
                <FileText size={13} />
                {boardApproved ? '✓ Board Approved' : 'Submit Board Recommendation'}
              </button>
              {phaseGate && phaseGate.requirements.filter((r) => !r.met).length > 0 && (
                <span className="text-[11px] text-text-muted">
                  {phaseGate.requirements.filter((r) => !r.met).length} task{phaseGate.requirements.filter((r) => !r.met).length > 1 ? 's' : ''} pending
                </span>
              )}
            </>
          ) : phase === 1 ? (
            <>
              {/* Phase 1 → 2: Present Pitch → Negotiate Fees */}
              <button
                onClick={() => setModal('pitch')}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-[var(--radius-md)] border text-[12px] font-medium transition-colors ${
                  pitchPresented
                    ? 'border-green-500/30 bg-green-500/5 text-green-400'
                    : 'border-accent-primary/40 bg-accent-soft text-text-accent hover:bg-accent-soft/80'
              ...
              >
                ...
                {pitchPresented ? '✓ Pitch Presented' : 'Present Pitch'}
              </button>

              <ChevronRight size={14} className="text-text-muted/40" />

              <button
                onClick={() => pitchPresented && setModal('fee')}
                disabled={!pitchPresented}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-[var(--radius-md)] border text-[12px] font-medium transition-colors ${
                  feeAgreed
                    ? 'border-green-500/30 bg-green-500/5 text-green-400'
                    : pitchPresented
                      ? 'border-accent-primary/40 bg-accent-soft text-text-accent hover:bg-accent-soft/80'
                      : 'border-border-subtle/40 text-text-muted/40 cursor-not-allowed'
                }`}
              >
                <Handshake size={13} />
                {feeAgreed
                  ? `✓ Fee Agreed (${agreedFeeTerms?.successFeePercent}%)`
                  : feeNegotiation?.status === 'in_progress'
                    ? `Negotiating (Round ${(feeNegotiation.rounds.length || 0) + 1})`
                    : 'Negotiate Fees'}
              </button>
            </>
          ) : (
            <>
              {/* Phase 2-9: Show engine gate requirements */}
              {phaseGate ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {phase === 7 && (
                    <Link
                      to="/final-offers"
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[var(--radius-md)] border text-[12px] font-medium transition-colors ml-2 ${
                        preferredBidderId
                          ? 'border-green-500/30 bg-green-500/5 text-green-400'
                          : 'border-accent-primary/40 bg-accent-soft text-text-accent hover:bg-accent-primary/20'
                      }`}
                    >
                      <Trophy size={12} />
                      {preferredBidderId ? '✓ Preferred Bidder Set' : 'Compare & Select Bidder'}
                    </Link>
                  )}
                  {phase === 8 && (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[var(--radius-md)] border text-[12px] font-medium transition-colors ml-2 ${
                        agreedSPATerms
                          ? 'border-green-500/30 bg-green-500/5 text-green-400'
                          : spaNegotiation?.status === 'in_progress'
                            ? 'border-state-warning/40 bg-state-warning/5 text-state-warning'
                            : 'border-accent-primary/40 bg-accent-soft text-text-accent hover:bg-accent-primary/20'
                      }`}
                    >
                      <ScrollText size={12} />
                      {agreedSPATerms ? '✓ SPA Agreed' : spaNegotiation?.status === 'in_progress' ? `SPA Round ${spaNegotiation.rounds.length + 1}` : 'Negotiate SPA'}
                    </button>
                  )}
                </div>
              ) : (
          ...
          {canAdvancePhase && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-[12px] font-semibold rounded-[var(--radius-md)] transition-colors"
              >
          ...
          {/* Budget request at end */}
          <div className="ml-auto flex items-center gap-2">
            {isBudgetLow && !pendingBudgetRequest && (
              <button
                onClick={() => setModal('budget')}
                className="flex items-center gap-1.5 px-3 py-2 rounded-[var(--radius-md)] border border-red-500/40 text-red-400 text-[11px] font-medium hover:bg-red-500/10 transition-colors"
              >
            ...
            {pendingBudgetRequest && (
              <span className="text-[11px] text-yellow-400 flex items-center gap-1">
                <Wallet size={11} /> Budget request pending
              </span>
            )}
            {phaseBudget && (
              <span className="text-[10px] text-text-muted">
                Phase budget: €{phaseBudget.phaseBase}k + €{phaseBudget.carryover}k carryover
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Panel title="Priority Actions" subtitle="Tasks requiring your attention" variant="accent">
                ...
                {activeTasks.slice(0, 4).map((task) => (
                  <Link key={task.id} to="/tasks" className="flex items-center justify-between p-2.5 rounded-[var(--radius-md)] bg-surface-default hover:bg-surface-hover transition-colors group">
                    <div className="flex items-center gap-3">
                      <StatusChip
                        label={task.status === 'recommended' ? 'Recommended' : 'Available'}
                        variant={task.status === 'recommended' ? 'accent' : 'default'}
                      />
                      <div>
            ...
            <Link to="/tasks" className="flex items-center gap-1 mt-3 text-[11px] text-text-accent hover:underline">
              View all tasks <ArrowRight size={12} />
            </Link>
          </Panel>

          {/* Active Workstreams */}
          <Panel title="Active Workstreams" headerRight={
            <Link to="/tasks" className="text-[11px] text-text-accent hover:underline">View tasks</Link>
          }>
              ...
              {activeWorkstreams.map((ws) => (
                <Link key={ws.id} to="/tasks" className="flex items-center gap-4 group hover:opacity-80 transition-opacity">
                  ...
                  <div className="flex-1">
                    <ProgressBar value={ws.progress} color="accent" />
                  </div>
          ...
          {/* Inbox Preview */}
          <Panel title="Inbox" subtitle={`${unreadEmails.length} unread`} headerRight={
            <Link to="/inbox" className="text-[11px] text-text-accent hover:underline">Open</Link>
          }>
        ...
        <div className="space-y-4">
          {/* Buyer Status */}
          <Panel title="Buyer Pipeline" headerRight={
            buyers.length > 0 ? <Link to="/buyers" className="text-[11px] text-text-accent hover:underline">View all</Link> : null
          }>
            {buyers.length === 0 ? (
          ...
          {/* Deliverables */}
          <Panel title="Deliverables" headerRight={
            <Link to="/deliverables" className="text-[11px] text-text-accent hover:underline">View all</Link>
          }>
              ...
              {deliverables.slice(0, 5).map((del) => (
                <Link key={del.id} to="/deliverables" className="block space-y-1 group">
                  <div className="flex items-center justify-between">
                    ...
                    <StatusChip
                      label={del.status.replace('_', ' ')}
                      variant={del.status === 'approved' ? 'success' : del.status === 'not_started' ? 'muted' : 'info'}
                    />
                  </div>
          ...
          {/* Risk / Alerts */}
          <Panel title="Active Risks" variant={activeRisks.some((r) => r.severity === 'high' || r.severity === 'critical') ? 'critical' : 'default'} headerRight={
            activeRisks.length > 0 ? <Link to="/risks" className="text-[11px] text-text-accent hover:underline">View all</Link> : null
          }>
          ...
          {/* Headlines */}
          <Panel title="Market Headlines" headerRight={
            <Link to="/market" className="text-[11px] text-text-accent hover:underline">Full view</Link>
          }>
            <div className="space-y-2">
