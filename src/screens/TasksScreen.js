            ...
            return (_jsxs("div", { className: `rounded-[var(--radius-md)] border transition-all duration-150 ${isExpanded ? 'bg-bg-panel/80 border-border-default' : 'bg-surface-default border-transparent hover:border-border-subtle'}`, children: [_jsx("button", { onClick: () => setExpandedTask(isExpanded ? null : task.id), className: "w-full p-3 text-left", children: _jsxs("div", { className: "flex items-start gap-3", children: [_jsx("span", { className: "shrink-0", children: task.status === 'locked' ? _jsx(Lock, { size: 14, className: "text-text-muted/40" }) :
                                        task.status === 'completed' ? _jsx(CheckCircle2, { size: 14, className: "text-state-success" }) :
                                            ...
                                            return unmetNames.length > 0
                                                ? _jsxs("span", { className: "text-state-warning", children: ["Blocked by: ", unmetNames.join(', ')] })
                                                : null;
                                        })()] }), isActionable && (_jsxs("button", { onClick: (e) => { e.stopPropagation(); startTask(task.id); }, disabled: !canAfford, className: `flex items-center gap-2 px-4 py-2 text-[12px] font-semibold rounded-[var(--radius-md)] transition-colors ${canAfford
                                        ? 'bg-accent-primary hover:bg-accent-hover text-white shadow-[var(--shadow-glow-soft)]'
                                        : 'bg-surface-default border border-border-subtle text-text-muted cursor-not-allowed'}`, children: [_jsx(Play, { size: 12 }), " ", canAfford ? 'Start Task' : `Need €${task.cost}k budget`] }))] }) }))] }, task.id));
        }) }));
    let taskContent;
    if (phase === 0) {
