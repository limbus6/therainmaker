...
export default function GameplayReviewBar() {
  ...
  const [selectedCheckpointId, setSelectedCheckpointId] = useState(
    () => REVIEW_CHECKPOINTS.find((cp) => cp.phase === currentPhase)?.id ?? REVIEW_CHECKPOINTS[0].id
  );
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 z-50 inline-flex items-center gap-2 rounded-full border border-border-subtle bg-surface-elevated/90 px-3.5 py-2 text-[12px] font-medium text-text-muted shadow-lg backdrop-blur-md transition-all hover:border-border-accent hover:text-text-primary hover:scale-105"
      >
  ...
  return (
    <section className="fixed bottom-4 right-4 z-50 w-[calc(100vw-2rem)] max-w-2xl rounded-[var(--radius-lg)] border border-border-subtle bg-surface-elevated/95 p-4 shadow-2xl backdrop-blur-lg">
      <div className="flex items-center justify-between gap-3 border-b border-border-subtle pb-3">
        <div className="flex items-center gap-2">
          <Bug size={16} className="text-text-accent" />
          <h3 className="text-[13px] font-semibold text-text-primary">Gameplay Review & Checkpoints</h3>
        </div>
        <button
          type="button"
      ...
      <div className="mt-3 space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] font-mono text-text-muted">Phase:</span>
            <select
              value={selectedPhase}
              onChange={(e) => {
                const p = Number(e.target.value) as PhaseId;
                setSelectedPhase(p);
                const first = REVIEW_CHECKPOINTS.find((c) => c.phase === p);
                if (first) setSelectedCheckpointId(first.id);
              }}
              className="rounded-[var(--radius-sm)] border border-border-subtle bg-bg-panel px-2 py-1 text-[12px] text-text-primary outline-none"
            >
              {Object.entries(PHASE_NAMES).map(([id, name]) => (
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] font-mono text-text-muted">Scenario:</span>
            <select
          ...
          <button
            type="button"
            onClick={handleJump}
            className="ml-auto inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] border border-border-accent bg-border-accent/10 px-3 py-1 text-[12px] font-medium text-text-accent transition-colors hover:bg-border-accent/20"
          >
            <SkipForward size={13} />
            <span>Jump</span>
          </button>
        ...
        {selectedCheckpoint && (
          <div className="grid gap-3 lg:grid-cols-[minmax(0,220px)_minmax(0,1fr)]">
            <div className="rounded-[var(--radius-md)] border border-border-subtle bg-surface-default p-2.5">
              <p className="text-[10px] font-mono uppercase tracking-wider text-text-muted">Scenario Context</p>
              <p className="mt-1 text-[12px] font-semibold text-text-primary">{selectedCheckpoint.label}</p>
              <p className="mt-1 text-[11px] text-text-secondary leading-snug">{selectedCheckpoint.description}</p>
            </div>

            <div className="rounded-[var(--radius-md)] border border-border-subtle bg-surface-default p-2.5">
              <div className="flex items-center justify-between gap-2 mb-2">
                <p className="text-[10px] font-mono uppercase tracking-wider text-text-muted">Review Note</p>
                <button
                  type="button"
                  onClick={handleSubmitReview}
                  disabled={isSubmitting}
                  className="inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] border border-border-default bg-bg-panel px-2.5 py-1 text-[11px] font-semibold text-text-primary hover:border-border-accent"
                >
                  <Send size={12} />
                  <span>{isSubmitting ? 'Sending...' : 'Save Note'}</span>
                </button>
              </div>
              <textarea
                value={reviewText}
                onChange={(e) => setReviewText(e.target.value)}
                placeholder="Describe bug or UX feedback for this scenario..."
                className="h-[56px] w-full rounded-[var(--radius-sm)] border border-border-default bg-bg-panel p-2 text-[11px] text-text-primary outline-none focus:border-border-accent"
              />
            </div>
          </div>
        )}
