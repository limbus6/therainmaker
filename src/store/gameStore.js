...
// ============================================
function deriveClientProfile(clientTrust, qualificationNotes) {
    ...
    if (clientTrust > 60 && positiveNotes)
        return 'serious_reasonable';
    if (clientTrust > 60 && !positiveNotes)
        return 'serious_demanding';
    if (clientTrust >= 40 && positiveNotes)
        return 'unsure_optimistic';
    return 'unsure_reluctant';
}
function buildClientNegotiationState(profile, expectedEV) {
    const configs = {
        serious_reasonable: {
            profile,
...
function resolveRetainerReaction(terms, clientState) {
    if (terms.playerRetainerType === 'none')
        return 'green';
    // unsure_reluctant and serious_demanding resist all retainer
    if (clientState.profile === 'unsure_reluctant' ||
    ...
    if (clientState.profile === 'unsure_optimistic')
        return 'red';
    return terms.playerRetainerAmount <= 4 ? 'yellow' : 'red';
}
function resolveSuccessFeeReaction(percent, clientState) {
    if (percent < clientState.reservationSuccessFeeMin)
        return 'red'; // too low — client expects more signal
    if (percent > clientState.reservationSuccessFeeMax)
        return 'red'; // too high
    // Yellow zone: within 0.5% of ceiling
    if (percent > clientState.reservationSuccessFeeMax - 0.5)
        return 'yellow';
    return 'green';
}
function resolveRatchetReaction(terms, clientState) {
...
function computeSatisfactionScore(rRetainer, rSuccessFee, rRatchet, clientState) {
    const score = (r) => r === 'green' ? 10 : r === 'yellow' ? 5 : 0;
    const weighted = score(rRetainer) * clientState.priorityRetainer +
...
function generateClientNote(profile, rRetainer, rSuccessFee, rRatchet, outcome) {
    if (outcome === 'accepted') {
        const map = {
...
// ============================================
function syncDeliverables(deliverables, tasks) {
...
// ============================================
function syncClient(client, resources) {
    return {
        ...client,
...
const PHASE_ZERO_DIMENSION_TASK_SUFFIX = {
    sector: '-sector',
    company: '-company',
    shareholder: '-shareholder',
    market: '-market',
};
function mapTaskStatusToInvestigationStatus(status) {
    if (status === 'completed')
        return 'completed';
...
function syncLeadsFromTasks(leads, tasks) {
...
// ============================================
function generateFinalOffers(buyers, momentum, week) {
    const BASE_EV = 120; // €M Solara baseline
    const offers = [];
    for (const buyer of buyers) {
        if (['dropped', 'excluded'].includes(buyer.status))
            continue;
        // EV range based on valuation posture
        const postureMultiplier = buyer.valuationPosture === 'aggressive' ? 1.12 :
        ...
        const structure = buyer.type === 'pe' ? (Math.random() > 0.5 ? 'mixed' : 'earnout_heavy') :
        ...
        const conditionality = buyer.ddFriction === 'high' ? 'heavy_conditions' :
...
// ============================================
function generateSPABuyerState(buyer) {
    const profile = buyer.type === 'pe' ? 'aggressive_buyer' :
    ...
    const jitter = (n, range) => Math.round(n + (Math.random() * range * 2 - range));
    return {
        profile,
...
function applyFeeProgressiveLocking(clientState, terms, reactions) {
    ...
    if (reactions[nextToLock] === 'green') {
        ...
        if (remaining.length > 0) {
            ...
            if (FEE_LOCK_HINTS[key])
                newHints.push(FEE_LOCK_HINTS[key]);
        }
    }
    else {
...
function applySPAProgressiveLocking(buyerState, terms, reactions) {
    ...
    if (reactions[nextToLock] === 'green') {
        newLocked.push(nextToLock);
        if (nextToLock === 'scope')
            updates.lockedWarrantyScope = terms.playerWarrantyScope;
        else if (nextToLock === 'cap')
            updates.lockedWarrantyCap = terms.playerWarrantyCap;
        else if (nextToLock === 'escrow')
            updates.lockedEscrowPercent = terms.playerEscrowPercent;
        const remaining = order.filter(c => !newLocked.includes(c));
        if (remaining.length > 0) {
            const key = `${buyerState.profile}/${nextToLock}->${remaining[0]}`;
            if (SPA_LOCK_HINTS[key])
                newHints.push(SPA_LOCK_HINTS[key]);
        }
    }
    else {
...
function evaluateSPARound(terms, buyerState, round, _resources) {
    ...
    const reactionCap = capGap <= 0 ? 'green' :
        capGap <= 8 ? 'yellow' : 'red';
    // Escrow reaction
    const escrowGap = buyerState.reservationEscrowPercent - terms.playerEscrowPercent;
    const reactionEscrow = escrowGap <= 0 ? 'green' :
        escrowGap <= 3 ? 'yellow' : 'red';
    // Indemnity reaction
    const reactionIndemnity = terms.playerSpecificIndemnity ? 'green' :
    ...
    let outcome = reds === 0 && yellows <= 1 ? 'accepted' :
    ...
    // Generate buyer note
    const notes = [];
    if (reactionScope === 'red')
        notes.push("Limited warranty scope is not acceptable — we need proper protection on fundamental reps.");
    else if (reactionScope === 'yellow')
        notes.push("We'd prefer broader warranty coverage, but we can work with this.");
    if (reactionCap === 'red')
        notes.push(`A ${terms.playerWarrantyCap}% cap is too low given deal complexity — our investment committee needs at least ${buyerState.reservationWarrantyCap}%.`);
    else if (reactionCap === 'yellow')
        notes.push("The cap is lower than we'd like, but we're not walking away over this.");
    if (reactionEscrow === 'red')
        notes.push(`Escrow of ${terms.playerEscrowPercent}% doesn't give us sufficient security — we need at least ${buyerState.reservationEscrowPercent}%.`);
    if (reactionIndemnity === 'red')
        notes.push("We need a specific indemnity for the identified risk — no indemnity, no deal.");
    if (notes.length === 0) {
...
export const useGameStore = create()(persist((set, get) => ({
    ...
    advanceWeek: () => {
        ...
        const updatedWorkstreams = state.workstreams.map((ws) => {
            ...
            const wsTasks = unlockedTasks.filter((t) => t.workstreamId === ws.id || (ws.id === 'preparation' && t.phase === state.phase && !t.workstreamId));
            if (wsTasks.length === 0)
                return ws;
        ...
        const resolvedBoardSub = result.resolvedBoardSubmission
            ? {
                ...
                status: result.resolvedBoardSubmission.approved ? 'approved' : 'rejected',
        ...
        set({
            ...
            budgetRequests: state.budgetRequests.map((req) => {
                ...
                if (resolved) {
                    return {
                        ...req,
                        status: resolved.approved ? 'approved' : 'rejected',
    ...
    markEmailRead: (emailId) => set((state) => ({
        emails: state.emails.map((e) => e.id === emailId && e.state === 'unread' ? { ...e, state: 'read' } : e),
    })),
    flagEmail: (emailId) => set((state) => ({
        emails: state.emails.map((e) => e.id === emailId ? { ...e, flagged: !e.flagged } : e),
    })),
    escalateEmail: (emailId) => set((state) => {
        const email = state.emails.find((e) => e.id === emailId);
        if (!email || email.escalated)
            return {};
        ...
        if (response?.resourceEffects) {
            ...
            for (const [key, delta] of Object.entries(response.resourceEffects)) {
                ...
                if (typeof current === 'number' && typeof delta === 'number') {
                    ...
                    newResources[k] = Math.max(0, Math.min(maxVal, current + variedDelta));
                }
            }
        }
        return {
            resources: normalizeResources(newResources),
            emails: state.emails.map((e) => e.id === emailId ? { ...e, state: 'resolved' } : e),
        };
    }),
    ...
    completeTask: (taskId) => set((state) => {
        ...
        const updatedWorkstreams = state.workstreams.map((ws) => {
            if (!ws.active)
                return ws;
            const wsTasks = unlockedUpdated.filter((t) => t.workstreamId === ws.id || (ws.id === 'preparation' && t.phase === state.phase && !t.workstreamId));
            if (wsTasks.length === 0)
                return ws;
            const completed = wsTasks.filter((t) => t.status === 'completed').length;
            return { ...ws, progress: Math.round((completed / wsTasks.length) * 100) };
    ...
    mitigateRisk: (riskId) => set((state) => {
        ...
        if (!risk || risk.mitigated)
            return {};
        // Mitigation costs resources
        const costMap = { low: 2, medium: 4, high: 6, critical: 8 };
        const cost = costMap[risk.severity] ?? 4;
        return {
            risks: state.risks.map((r) => r.id === riskId ? { ...r, mitigated: true, probability: Math.max(0, r.probability - 20) } : r),
            resources: normalizeResources({
                ...state.resources,
    ...
    executeRiskMitigationPlan: (riskId, planId) => set((state) => {
        ...
        if (!plan)
            return {};
        if (state.resources.budget < plan.budgetCost)
            return {};
        if (state.resources.teamCapacity < plan.capacityCost)
            return {};
        let baseResources = {
            ...state.resources,
        ...
        const catastrophicFail = !!plan.catastrophicFailureChance && Math.random() < plan.catastrophicFailureChance;
        if (catastrophicFail) {
            baseResources = normalizeResources({ ...baseResources, clientTrust: 0, dealMomentum: 0, riskLevel: 100 });
            return {
                resources: baseResources,
                gameComplete: true,
                collapseReason: 'client_walked',
                collapseHeadline: plan.catastrophicHeadline ?? 'Client Walked',
                collapseDescription: plan.catastrophicDescription ??
                    'The mitigation move backfired and the client terminated the engagement.',
                toasts: [
                    ...state.toasts,
                    { id: Math.random().toString(36).substr(2, 9), message: 'Mitigation backfired: client exited the process.', type: 'danger' },
                ],
            };
        }
        ...
        for (const [key, delta] of Object.entries(resourceDeltas)) {
            ...
            if (typeof current === 'number' && typeof delta === 'number') {
                mergedResources[k] = current + delta;
            }
        }
        const nextRiskProbability = Math.max(0, Math.min(100, risk.probability + probabilityDelta));
        const boardSubmissionUpdate = success && plan.boardRecommendation && !state.boardSubmission
            ? {
    ...
    resolveBudgetRequest: (id, approved, approvedAmount) => set((state) => {
        ...
        return {
            budgetRequests: state.budgetRequests.map((r) => r.id === id
                ? { ...r, status: approved ? 'approved' : 'rejected', approvedAmount: injected }
                : r),
            resources: approved
    ...
    hireStaffer: (profile) => set((state) => {
        ...
        if (!cfg)
            return {};
        if (state.resources.budget < cfg.hireCost)
            return {};
        const newMember = {
    ...
    startFeeNegotiation: () => set((state) => {
        if (!state.feeNegotiation?.pitchPresented)
            return {};
        return { feeNegotiation: { ...state.feeNegotiation, status: 'in_progress' } };
    }),
    submitFeeRound: (terms) => set((state) => {
        if (!state.feeNegotiation || state.feeNegotiation.status !== 'in_progress')
            return {};
        ...
        const reactionRetainer = clientState.lockedComponents.includes('retainer')
            ? 'green' : resolveRetainerReaction(effectiveTerms, clientState);
        const reactionSuccessFee = clientState.lockedComponents.includes('successFee')
            ? 'green' : resolveSuccessFeeReaction(effectiveTerms.playerSuccessFeePercent, clientState);
        const reactionRatchet = clientState.lockedComponents.includes('ratchet')
        ...
        const patienceDrain = (reactionRetainer === 'red' ? 25 * (clientState.priorityRetainer / 10) : 0) +
        ...
        const outcome = accepted ? 'accepted' : (failed ? 'rejected' : 'counter');
        ...
        const lockUpdates = outcome === 'counter'
            ...
            : {};
        const updatedClientState = { ...clientState, patienceRemaining: newPatience, ...lockUpdates };
        if (accepted) {
    ...
    acceptFeeTerms: () => set((state) => {
        ...
        if (!lastRound || !state.feeNegotiation)
            return {};
        const ev = state.client.valuationExpectationEV ?? 100;
    ...
    submitSPARound: (terms) => set((state) => {
        ...
        const reactionEscrow = lockedComponents.includes('escrow') ? 'green' : rawResult.reactionEscrow;
        const reactionIndemnity = lockedComponents.includes('indemnity') ? 'green' : rawResult.reactionIndemnity;
        // Recompute outcome with overridden reactions
        const reds = [reactionScope, reactionCap, reactionEscrow, reactionIndemnity].filter(r => r === 'red').length;
        const yellows = [reactionScope, reactionCap, reactionEscrow, reactionIndemnity].filter(r => r === 'yellow').length;
        const patience = neg.buyerState.patienceRemaining;
        let outcome = reds === 0 && yellows <= 1 ? 'accepted' : reds >= 2 || patience < 20 ? 'rejected' : 'counter';
        if (round >= 3 && reds >= 1)
            outcome = 'rejected';
        const result = { ...rawResult, reactionScope, reactionCap, reactionEscrow, reactionIndemnity, outcome };
        const newRound = { round, ...effectiveTerms, ...result };
        ...
        const agreedTerms = result.outcome === 'accepted' ? {
    ...
    setDataroomAccess: (categoryId, level) => set((state) => {
        ...
        return {
            dataroomCategories: state.dataroomCategories.map((c) => c.id === categoryId ? { ...c, accessLevel: level } : c),
            resources: normalizeResources(newResources),
        };
    }),
    // Toast actions
    ...
    executeMitigationAction: (threatId, action) => set((state) => {
        ...
        if (!cfg)
            return {};
        if (state.resources.budget < cfg.budgetCost)
            return {};
