...
export function createInitialEventDirectorState() {
    return {
        tensionScore: 50,
        recoveryCounter: 0,
 ...
 */
export function selectEvents(pool, state, directorState, daysElapsed, rng, maxEvents = 2) {
    ...
    const activeMajorRisks = (state.risks || []).filter((r) => !r.mitigated && !r.retired && (r.severity === 'high' || r.severity === 'critical')).length;
    ...
    const eligiblePool = pool.filter((tmpl) => {
        // Phase check
        if (!tmpl.phases.includes(state.phase))
            return false;
