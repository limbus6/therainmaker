...
function calculateFinancialScore(state) {
    ...
    if (dealClosed) {
        if (selectedOffer && preferredBuyer) {
            ...
            const conditionDiscount = selectedOffer.conditionality === 'clean' ? 1.0 :
            ...
            const structureScore = selectedOffer.structure === 'full_cash' ? 1.0 :
                selectedOffer.structure === 'mixed' ? 0.75 : 0.5;
            const combinedCashScore = (cashCertainty + structureScore) / 2;
            const conditionScore = selectedOffer.conditionality === 'clean' ? 1.0 :
