...
import { selectEvents, createInitialEventDirectorState } from './eventDirector';
...
function createGameplayDirectorSignal(state) {
    ...
    const deadlinePressure = state.phaseDeadline && state.day <= state.phaseDeadline
        ...
        : 0;
    const resourceStress = Math.max(0, 55 - state.resources.dealMomentum) * 0.35 +
    ...
    const tensionBand = pressureScore >= 72 ? 'danger'
...
function resolveTaskProgress(task, tempAllocations = [], daysToAdvance = 7, budget = 0, paceCompletionMult = 1.0) {
    const alloc = tempAllocations.find((a) => a.taskId === task.id && (a.phase === undefined || a.phase === task.phase));
...
function checkHiddenWorkload(completedTasks, directorSignal) {
...
function rollCriticalOutcomes(completedTasks, morale = 50, directorSignal) {
    const outcomes = [];
    const successPool = {
        deliverable: [
...
function applyResourceNoise(resourceChanges, state, directorSignal) {
    ...
    const currentMorale = noised.morale ?? state.resources.morale;
    ...
    const currentMomentum = noised.dealMomentum ?? state.resources.dealMomentum;
...
function calculateResourceConsumption(inProgressTasks, resources, daysToAdvance = 7) {
...
function calculateStateChanges(completedTasks, resources) {
    let momentumDelta = 0;
    let trustDelta = 0;
    for (const task of completedTasks) {
...
function generateSummary(result, _week, directorSignal) {
    const parts = [];
...
// ============================================
const INTEREST_ORDER = ['cold', 'lukewarm', 'warm', 'hot', 'on_fire'];
function progressBuyers(buyers, completedTasks, phase, momentum) {
    if (buyers.length === 0)
        return { buyers, changes: [] };
    const changes = [];
...
function rollEvents(state, daysToAdvance, directorSignal) {
    ...
    const { selectedTemplates, nextDirectorState } = selectEvents(directorPool, state, directorState, daysToAdvance, rng, maxEvents);
    const result = {
        events: [],
        resourceEffects: {},
        risks: [],
    ...
    for (const template of selectedTemplates) {
        ...
        if (firedIds.has(generated.event.id))
            continue;
        ...
        if (generated.resourceEffects) {
            for (const [k, v] of Object.entries(generated.resourceEffects)) {
                result.resourceEffects[k] = (result.resourceEffects[k] ?? 0) + v;
            }
        }
...
// ============================================
export function resolveWeek(state, daysToAdvance = 7) {
    ...
    const contractorSpend = state.tempCapacityAllocations
        .filter((alloc) => ((alloc.phase === undefined || alloc.phase === state.phase) &&
    ...
    if (contractorSpend > 0) {
        resourceConsumption.budget = Math.max(0, Math.round((state.resources.budget - contractorSpend) * 100) / 100);
    }
    ...
    for (const crit of criticalOutcomes) {
        for (const [key, delta] of Object.entries(crit.bonus)) {
            const k = key;
            const current = resourceChanges[k] ?? state.resources[k];
            if (typeof current === 'number' && typeof delta === 'number') {
                ...
                resourceChanges[k] = Math.max(0, Math.min(maxVal, current + delta));
            }
        }
    }
    ...
    for (const req of state.budgetRequests) {
        if (req.status === 'pending') {
            ...
            if (approved) {
                if (trustFactor > 0.2)
                    reasoning = 'Strong client trust supports additional investment.';
                else if (phaseFactor > 0)
                    reasoning = 'Deal progression justifies additional resources.';
                else
                    reasoning = 'Budget allocation approved by committee.';
            }
            else {
                if (riskFactor > 0.1)
                    reasoning = 'Risk level too high for additional spend.';
                else if (state.resources.budget < req.amount)
                    reasoning = 'Insufficient budget available.';
                else
                    reasoning = 'Committee prefers to focus on core workstreams.';
            }
    ...
    let resolvedBoardSubmission = null;
    if (state.phase === 0 && state.boardSubmission && state.boardSubmission.status === 'pending') {
        ...
        if (state.boardSubmission.recommendation === 'decline')
            approvalChance -= 0.40;
        // Penalty if submitting with very few notes (unprepared)
        if (qualNotes.length === 0)
            approvalChance -= 0.30;
        else if (qualNotes.length === 1)
            approvalChance -= 0.10;
    ...
    for (const [key, value] of Object.entries(eventResult.resourceEffects)) {
        ...
        if (k === 'budget') {
            ...
            resourceChanges.budget = (resourceChanges.budget ?? state.resources.budget) + value;
            ...
            if (typeof current === 'number') {
                ...
                resourceChanges[k] = Math.max(0, Math.min(maxVal, current + value));
            }
        }
    }
    ...
    if (state.phase === 6 &&
        ...
        const ddBuyers = updatedBuyersAfterDeadline.filter(b => !['dropped', 'excluded'].includes(b.status) && !b.bindingOfferSubmitted);
        for (const buyer of ddBuyers) {
            ...
            if (dataroomScore < 0.5)
                dropoutP += 0.30;
            else if (dataroomScore < 0.75)
                dropoutP += 0.12;
            ...
            if (buyer.ddFriction === 'high')
                dropoutP += 0.15;
            else if (buyer.ddFriction === 'medium')
                dropoutP += 0.05;
            ...
            if (submits) {
                ...
                const reason = riskPenalty > 0.20 ? 'material issues identified during due diligence that were not adequately disclosed in the data room'
...
// ============================================
export function calcDaysToAdvance(state) {
...
export function checkDealCollapse(state) {
    const { resources, buyers, phase } = state;
    const none = { collapsed: false, reason: null, headline: '', description: '' };
...
export function checkPhaseGate(state) {
    const { phase, tasks } = state;
    const phaseTasks = tasks.filter((t) => t.phase === phase);
    const completedCount = phaseTasks.filter((t) => t.status === 'completed').length;
    const totalCount = phaseTasks.length;
    const currentTaskCompleted = (taskId) => phaseTasks.find((task) => task.id === taskId)?.status === 'completed';
    switch (phase) {
        case 0: { // Deal Origination → Pitch & Mandate
            const anyLeadInvestigated = state.leads.some(l => l.investigation.sector === 'completed' ||
        ...
        case 5: { // Non-Binding Offers → Due Diligence
            ...
            const clientSelectedDD = tasks.some((t) => (t.id === 'task-75' ||
                t.id === 'task-76' ||
                t.name.toLowerCase().includes('advancement recommendation') ||
                t.name.toLowerCase().includes('dd candidate')) && t.status === 'completed');
            return {
...
// ============================================
export function unlockTasks(tasks) {
    return tasks.map((task) => {
        if (task.status !== 'locked' || !task.dependencies)
            return task;
...
//# sourceMappingURL=weekEngine.js.map