...
function makeMission(id, requiredActionIds) {
    return {
        id,
        phase: 2,
        title: `Mission ${id}`,
...
function makeTask(id, status) {
    return { id, name: id, description: '', phase: 2, category: 'internal', status, cost: 0, work: 3, complexity: 'low', effectSummary: '' };
}
const missions = [
...
describe('getMissionProgress', () => {
    ...
    it('ignores tasks from other phases with the same id', () => {
        const otherPhaseTask = { ...makeTask('t1', 'completed'), phase: 3 };
...
//# sourceMappingURL=missionProgress.test.js.map