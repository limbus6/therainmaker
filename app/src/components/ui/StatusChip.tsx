...
export default function StatusChip({ label, variant = 'default', size = 'sm' }: StatusChipProps) {
  const sizeClass = size === 'sm' ? 'text-[10px] px-2 py-0.5' : 'text-[11px] px-2.5 py-1';
  return (
    <span className={`inline-flex items-center rounded-[var(--radius-sm)] border font-mono uppercase tracking-wider font-medium ${sizeClass} ${variantStyles[variant]}`}>
      {label}
    </span>
  );
}
